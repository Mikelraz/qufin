...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ppy", type=int, default=252, help="Trading periods per year")
...
def _download_prices(tickers: list[str], start: str, end: str) -> pl.DataFrame:
    ...
    close = close.dropna()
    pandas_df = close.reset_index()
    # reset_index() puts the DatetimeIndex first; rename it regardless of its name
    pandas_df = pandas_df.rename(columns={pandas_df.columns[0]: "date"})
...
def _result_label(res: OptimizationResult, all_results: tuple[OptimizationResult, ...]) -> str:
...
def main() -> None:
    ...
    methods = (
        ["min-variance", "max-sharpe", "risk-parity"] if args.method == "all" else [args.method]
    )
    ...
    if "min-variance" in methods:
        ...
        m = portfolio_metrics(res.weights, mat, risk_free_rate=args.rfr, periods_per_year=args.ppy)
